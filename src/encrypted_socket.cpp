...
void encrypted_socket::start_kx(optional<connect_callback> callback) {
    ...
    boost::asio::async_write(*this->socket, buffer, [this](boost::system::error_code ec, size_t n_written) {
        if (ec) {
...
void encrypted_socket::finish_kx(optional<connect_callback> callback) {
    this->start_read_msg([this, callback](shared_ptr<uint8_t[]> buffer, size_t msg_len) {
...
void encrypted_socket::start_msg_loop() {
    this->start_read_msg([this](shared_ptr<uint8_t[]> data, size_t size) {
        auto nonce = data.get();
        auto ciphertext = nonce + crypto_aead_chacha20poly1305_IETF_NPUBBYTES;
        auto ciphertext_len = this->msg_size - crypto_aead_chacha20poly1305_IETF_NPUBBYTES;
        auto decrypted = make_shared<uint8_t[]>(size);
        unsigned long long decrypted_len;

        sodium_mprotect_readonly(this->session_key);
        if (crypto_aead_chacha20poly1305_ietf_decrypt(decrypted.get(), &decrypted_len, 
...
void encrypted_socket::send(unique_ptr<uint8_t[]> data, size_t data_size) {
    ...
    os.write(reinterpret_cast<const char *>(nonce.get()), crypto_aead_chacha20poly1305_IETF_NPUBBYTES);
...
unique_ptr<uint8_t[]> encrypted_socket::generate_nonce() {
    this->nonce_counter++;
    auto nonce = make_unique<uint8_t[]>(crypto_aead_chacha20poly1305_IETF_NPUBBYTES);
    crypto_kdf_derive_from_key(nonce.get(), crypto_aead_chacha20poly1305_IETF_NPUBBYTES, this->nonce_counter, "noncekey", this->nonce_seed);
...
void encrypted_socket::start_read_msg(data_callback callback) {
    boost::asio::async_read(*this->socket, *this->buffer, boost::asio::transfer_exactly(2), [this, callback](boost::system::error_code ec, size_t n_bytes) {
        if (!ec) {
            ...
            extension.LogMessage("Error reading packet: %s", ec.message().c_str());
        }
    });
}

void encrypted_socket::do_read(data_callback callback) {
    boost::asio::async_read(*this->socket, *this->buffer, boost::asio::transfer_exactly(this->msg_size), [this, callback](boost::system::error_code ec, size_t n_bytes) {
        if (!ec) {
