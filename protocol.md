...
game servers, and should probably not be used for anything of significant importance.
...
and the [Double Ratchet Algorithm](https://signal.org/docs/specifications/doubleratchet/#overview), albeit significantly simplified from
...
of $`E_A`$ and $`E_B`$ in transit by is guaranteed by computing their signature with a derived shared secret key $`K_d`$.
```math
\begin{aligned}
...
nonce derived from the chain key $`C_{i,j}`$.
```math
\begin{aligned}
